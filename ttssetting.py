...
settings['speaking_rate'] = inputWithCheck(
    "音声速度の倍率(0.25～4.0)倍", settings['speaking_rate'], 0.25, 4.0)
settings['pitch'] = inputWithCheck(
    "音声の音程(-20.0～20.0)セミトーン", settings['pitch'], -20.0, 20.0)
settings['volume_gain_db'] = inputWithCheck(
    "音声の音量(-96.0～16.0)dB", settings['volume_gain_db'], -96.0, 16.0)
...
while(True):
    ...
    if selected_str != 'None':
        if 'None' in available_profiles:
            available_profiles.remove('None')
    else:
        break
    # end if
    while(True):
...
while(True):
    n = input("この設定につける名前:")
    if re.match(SETTING_FILE_NAME_PATTERN, n):
        break
